...
public class EmployeeRestController {
    ...
    public Response createEmployee(Employee employee) throws URISyntaxException{
        ...
        Set<ConstraintViolation<Employee>> violations = validator.validate(employee); //have to findout
        Employee e = EmployeeDB.getEmployee(employee.getId());
        if(violations.size()>0){
            ArrayList<String> validationMessages = new ArrayList<>();
            for(ConstraintViolation<Employee> violation : violations){
                validationMessages.add(violation.getPropertyPath().toString() + " : " + violation.getMessage().toString());
            }
